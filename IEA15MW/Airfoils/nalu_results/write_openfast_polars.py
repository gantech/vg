...
vg_nalu_polars = AirfoilTableDB('static_vg/airfoils_smooth_rey10000000.yaml')
...
Path('../../OpenFAST_NaluPolars/Modified_Airfoils').mkdir(exist_ok=True)
for i,s in enumerate(of_ad15_blade[:,0]):
    ui = np.argmin( tbyc > tbyc_interp(s)) #Upper station index
    li = ui-1 #Lower station index
    interp_fac = (tbyc_interp(s) - tbyc[ui])/(tbyc[li] - tbyc[ui])

    ui_afp = vg_nalu_polars.get_airfoil_data(t.airfoil_labels[ui])
    li_afp = vg_nalu_polars.get_airfoil_data(t.airfoil_labels[li])
    ...
    afp_data = AirfoilTable(afp_extrap.alpha, cl=afp_extrap.cl, cd=afp_extrap.cd, cm=afp_extrap.cm)
    afp_data.write_openfast_polar('../../OpenFAST_NaluPolars/Modified_Airfoils/IEA-15-240-RWT_AeroDyn15_Polar_{:02d}.dat'.format(i))
