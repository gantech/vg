...
class VG:
    def __init__(self, template_file='vg_template_coarse.dat',
                 beta=0, l=1.0, h=1.0, x0=0.0, y0=0.0, z0=0.0, normal_vec=[0,0,1.0]):
        ...
        nodes = pd.read_csv(template_file,sep='\s+',skiprows=1,header=None,
                            nrows=10) * np.array([l,h,0.0])
        nodes = np.loadtxt(template_file,skiprows=1,max_rows=10) * np.array([l,h,0.0])

        cell_ids = pd.read_csv(template_file,sep='\s+',skiprows=12
                               ,header=None) - 1
        cell_ids = np.loadtxt(template_file,skiprows=12,dtype=int)-1
