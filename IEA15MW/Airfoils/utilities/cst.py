...
class CSTAirfoil(CST2D):
    ...
    def __init__(self, airfoil, order=8, shape_class='airfoil'):
        ...
        if (shape_class == 'ellipse'):
            self._cst_lower = -np.ones(order+1)
            self._cst_upper = np.ones(order+1)
    ...
    def __call__(self, xinp, te_upper=None, te_lower=None):
...
class AirfoilShape:
    ...
    def from_cst_parameters(cls, cst_lower, te_lower, cst_upper, te_upper,
        ...
        x_c = -np.cos(np.arange(0,np.pi+0.005,np.pi*0.01))*0.5+0.5
